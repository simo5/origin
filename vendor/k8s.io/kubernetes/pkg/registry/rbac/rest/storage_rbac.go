...
func (p RESTStorageProvider) PostStartHook() (string, genericapiserver.PostStartHookFunc, error) {
	return PostStartHookName, GetPostStartHook(bootstrappolicy.Policy()), nil
}

func GetPostStartHook(bootstrapPolicyData *bootstrappolicy.PolicyData) genericapiserver.PostStartHookFunc {
	return func(hookContext genericapiserver.PostStartHookContext) error {
		...
		err := wait.Poll(1*time.Second, 30*time.Second, func() (done bool, err error) {
			...
			for _, clusterRole := range bootstrapPolicyData.ClusterRoles {
			...
			for _, clusterRoleBinding := range bootstrapPolicyData.ClusterRoleBindings {
			...
			for namespace, roles := range bootstrapPolicyData.Roles {
			...
			for namespace, roleBindings := range bootstrapPolicyData.RoleBindings {
